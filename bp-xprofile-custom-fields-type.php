<?php
/**
 * Plugin Name: Buddypress XProfile Custom Field Types Reloaded
 ...
 * Version:     2.6.6
 * Text Domain: bp-xprofile-custom-fields
...
if ( ! class_exists( 'SQ_BP_XProfile' ) ) {
	class SQ_BP_XProfile {
		...
		public function __construct() {
			...
			add_filter( 'bp_get_the_profile_field_value', array(
				$this,
				'bxcft_remove_autolink_filter_from_buddypress'
			), 7, 2 );
			add_filter( 'bp_get_the_profile_field_value', array( $this, 'bxcft_get_field_value' ), 10, 3 );
			add_filter( 'bp_get_the_profile_field_value', array(
				$this,
				'bxcft_restore_autolink_filter_from_buddypress'
			), 11, 2 );
			...
			add_filter( 'bps_field_type_for_search_form', array(
				$this,
				'bxcft_standard_fields_type_for_search_form'
			) );
			...
			add_filter( 'bp_xprofile_set_field_data_pre_validate', array(
				$this,
				'bxcft_xprofile_set_field_data_pre_validate'
			), 10, 3 );
		}
		...
		public function init() {
			...
			$this->images_max_filesize      = apply_filters( 'bxcft_images_max_filesize', SQ_BP_XProfile::BXCFT_MAX_FILESIZE );
			$this->files_ext_allowed        = apply_filters( 'bxcft_files_ext_allowed', array(
				'doc',
				'docx',
				'pdf'
			) );
			$this->files_max_filesize       = apply_filters( 'bxcft_files_max_filesize', SQ_BP_XProfile::BXCFT_MAX_FILESIZE );
		...
		public function admin_init() {
			if ( is_admin() && get_option( 'bxcft_activated' ) == 1 ) {
				...
				if ( ! $this->compare_versions( array_pop( $version_bp ), '2.5' ) ) {
					...
					if ( ! array_pop( $version_bp ) ) {
						$notices[] = __( 'BuddyPress Xprofile Custom Fields Type plugin needs <b>BuddyPress</b> plugin, please install and active BuddyPress.', 'bp-xprofile-custom-fields' );
					} else {
						$notices[] = __( 'BuddyPress Xprofile Custom Fields Type plugin needs <b>BuddyPress 2.5</b>, please install or upgrade BuddyPress.', 'bp-xprofile-custom-fields' );
					}
					...
					$notices   = get_option( 'bxcft_notices' );
					$notices[] = __( 'BuddyPress Xprofile Custom Fields Type plugin needs Buddypress Xprofile Component. Please enable Xprofile first.', 'bp-xprofile-custom-fields' );
					update_option( 'bxcft_notices', $notices );
					delete_option( 'bxcft_activated' );
				}
		...
		public function bxcft_remove_autolink_filter_from_buddypress( $value, $type = '' ) {
			if ( in_array( $type, array_keys( $this->bxcft_field_types ) )
			     && ! SQ_BP_XProfile::is_autolink_filter_removed()
			) {
						...
						if ( ( $field->type == 'image' || $field->type == 'file' ) &&
							...
							if ( $field->is_required && $filesize <= 0 ) {
								...
								if ( $field->type == 'image' ) {
									if ( ! in_array( $ext, $this->images_ext_allowed ) ) {
										$bp->signup->errors[ 'field_' . $field_id ] = sprintf( __( 'Image type not allowed: (%s).', 'bp-xprofile-custom-fields' ), implode( ',', $this->images_ext_allowed ) );
									} elseif ( $filesize > $this->images_max_filesize ) {
										$bp->signup->errors[ 'field_' . $field_id ] = sprintf( __( 'Max image upload size: %s MB.', 'bp-xprofile-custom-fields' ), $this->images_max_filesize );
									}
								} elseif ( $field->type == 'file' ) {
									if ( ! in_array( $ext, $this->files_ext_allowed ) ) {
										$bp->signup->errors[ 'field_' . $field_id ] = sprintf( __( 'File type not allowed: (%s).', 'bp-xprofile-custom-fields' ), implode( ',', $this->files_ext_allowed ) );
									} elseif ( $filesize > $this->files_max_filesize ) {
										$bp->signup->errors[ 'field_' . $field_id ] = sprintf( __( 'Max file upload size: %s MB.', 'bp-xprofile-custom-fields' ), $this->files_max_filesize );
									}
								}
							}
							...
							if ( $max_age > 0 ) {
								...
								if ( class_exists( DateTime ) ) {
									...
									if ( $age->y < $max_age ) {
										$bp->signup->errors[ 'field_' . $field_id ] = sprintf( __( 'You have to be at least %s years old.', 'bp-xprofile-custom-fields' ), $max_age );
									}
								}
							}
						}
			...
			if ( $field->type == 'image' || $field->type == 'file' && isset( $_FILES[ 'field_' . $field_id ] ) ) {
				...
				if ( isset( $_FILES[ 'field_' . $field_id ] ) && $filesize > 0 ) {
					...
					if ( $field->type == 'image' ) {
						...
						if ( ! in_array( $ext, $ext_allowed ) ) {
							bp_core_add_message( sprintf( __( 'Image type not allowed: (%s).', 'bp-xprofile-custom-fields' ), implode( ',', $this->images_ext_allowed ) ), 'error' );
							bp_core_redirect( trailingslashit( bp_displayed_user_domain() . $bp->profile->slug . '/edit/group/' . bp_action_variable( 1 ) ) );
						} elseif ( $filesize > $this->images_max_filesize ) {
							bp_core_add_message( sprintf( __( 'Max image upload size: %s MB.', 'bp-xprofile-custom-fields' ), $this->images_max_filesize ), 'error' );
							bp_core_redirect( trailingslashit( bp_displayed_user_domain() . $bp->profile->slug . '/edit/group/' . bp_action_variable( 1 ) ) );
						} else {
							// Delete previous image.
							if ( isset( $field->value ) && ! empty( $field->value ) && file_exists( $uploads['basedir'] . $field->value ) ) {
								wp_delete_file( $uploads['basedir'] . $field->value );
							}
						}
					} elseif ( $field->type == 'file' ) {
						$ext_allowed = $this->files_ext_allowed;
						if ( ! in_array( $ext, $ext_allowed ) ) {
							bp_core_add_message( sprintf( __( 'File type not allowed: (%s).', 'bp-xprofile-custom-fields' ), implode( ',', $this->files_ext_allowed ) ), 'error' );
							bp_core_redirect( trailingslashit( bp_displayed_user_domain() . $bp->profile->slug . '/edit/group/' . bp_action_variable( 1 ) ) );
						} elseif ( $filesize > $this->files_max_filesize ) {
							bp_core_add_message( sprintf( __( 'Max file upload size: %s MB.', 'bp-xprofile-custom-fields' ), $this->files_max_filesize ), 'error' );
							bp_core_redirect( trailingslashit( bp_displayed_user_domain() . $bp->profile->slug . '/edit/group/' . bp_action_variable( 1 ) ) );
						} else {
				...
				if ( $max_age > 0 ) {
					...
					if ( class_exists( DateTime ) ) {
						...
						$age       = $now->diff( $birthdate );
						if ( $age->y < $max_age ) {
							bp_core_add_message( sprintf( __( 'You have to be at least %s years old.', 'bp-xprofile-custom-fields' ), $max_age ), 'error' );
							bp_core_redirect( trailingslashit( bp_displayed_user_domain() . $bp->profile->slug . '/edit/group/' . bp_action_variable( 1 ) ) );
						}
					}
				}
		...
		public function bxcft_special_fields_data_for_search_form( $f ) {
			...
			switch ( $f->type ) {
				...
				case 'checkbox_acceptance':
					$f->values  = isset ( $request[ $f->code ] ) ? (array) $request[ $f->code ] : array();
					$f->options = array(
						0 => __( 'no', 'bp-xprofile-custom-fields' ),
						1 => __( 'yes', 'bp-xprofile-custom-fields' )
					);
					$f->display = 'radio';
					break;
				...
				case 'file':
					$f->values  = isset ( $request[ $f->code ] ) ? (array) $request[ $f->code ] : array();
					$f->options = array(
						1 => __( 'Not empty', 'bp-xprofile-custom-fields' )
					);
					$f->display = 'checkbox';
					break;
			}
		...
		public function bxcft_update() {
			...
			if ( ! empty( $locale ) ) {
				...
				if ( file_exists( $mofile ) ) {
					load_textdomain( "bp-xprofile-custom-fields", $mofile );
				}
			}
...
if ( class_exists( 'SQ_BP_XProfile' ) ) {
	register_activation_hook( __FILE__, array( 'SQ_BP_XProfile', 'activate' ) );
	register_deactivation_hook( __FILE__, array( 'SQ_BP_XProfile', 'deactivate' ) );
	$bxcft_plugin = new SQ_BP_XProfile();
}